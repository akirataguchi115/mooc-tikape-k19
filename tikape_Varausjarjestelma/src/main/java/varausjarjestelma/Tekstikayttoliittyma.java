...
public class Tekstikayttoliittyma {
    ...
    private void lisaaHuone(Scanner s) {
        ...
        Huone huone = new Huone(1, tyyppi, numero, hinta);
        try {
    ...
    private void listaaHuoneet() {
        ...
        List<Huone> huoneet
                = jdbcTemplate.query("SELECT * FROM Huone",
                        (rs, rowNum) -> new Huone(rs.getInt("id"), rs.getString("tyyppi"),
                                rs.getInt("numero"), rs.getInt("paivahinta"))
                );
        huoneet.forEach(huone -> {
            System.out.println(huone.getTyyppi() + ", " + huone.getNumero() + ", " + huone.getPaivahinta() + " euroa");
...
//        System.out.println("Commodore, 128, 229 euroa");
    ...
    private void haeHuoneita(Scanner s) {
        ...
        if (maksimihinta.isEmpty() && tyyppi.isEmpty()) {
            List<Huone> huoneet
                    = jdbcTemplate.query("SELECT * FROM Huone "
                            + "LEFT JOIN HuoneTilaus ON Huone.id = HuoneTilaus.huone_id "
                            + "LEFT JOIN Tilaus ON Tilaus.id = HuoneTilaus.tilaus_id "
                            + "WHERE Tilaus.lopetuspvm < ? AND Tilaus.aloituspvm > ? "
                            + "OR Tilaus.aloituspvm = NULL;",
                            (rs, rowNum) -> new Huone(rs.getInt("id"), rs.getString("tyyppi"),
                                    rs.getInt("numero"), rs.getInt("paivahinta")),
                            alku, loppu);

            System.out.print("vapaat huoneet: ");
            huoneet.forEach(huone -> {
                System.out.println(huone.getTyyppi() + ", " + huone.getNumero() + ", " + huone.getPaivahinta() + " euroa");
